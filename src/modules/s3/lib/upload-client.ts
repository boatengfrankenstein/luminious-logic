...
export class S3Client {
  ...
  private async uploadWithProgress(
    ...
    return new Promise((resolve, reject) => {
      ...
      xhr.onerror = () => {
        reject(
          new UploadError("Network error during upload", {
            status: xhr.status,
  ...
  /**
   * Retry upload on transient network issues
   * @param file the file to upload
   * @param uploadUrl signed upload URL
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt += 1) {
      try {
        ...
        if (!isTransient || attempt === MAX_RETRIES) {
          throw new UploadError(
            `Upload failed after ${attempt} attempt${attempt > 1 ? "s" : ""}`,
  ...
  async upload({
    ...
    try {
      ...
      await this.uploadWithRetry(file, uploadUrl, onProgress);
